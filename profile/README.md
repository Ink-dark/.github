# 你好，我是 Ink-dark 👋 / Hi, I'm Ink-dark

> 墨染柒DarkSeven — 写点疯狂的代码，做点有趣的项目。
> DarkSeven — Writing crazy code, building fun projects.

[![GitHub followers](https://img.shields.io/github/followers/Ink-dark?style=social)](https://github.com/Ink-dark)
[![GitHub stars](https://img.shields.io/github/stars/Ink-dark?style=social)](https://github.com/Ink-dark)

---

## 🧠 关于我 / About Me

一个脑子里塞满疯狂点子的开发者。喜欢从零造轮子，用代码把想法变成现实。

A developer with a head full of crazy ideas. I love building things from scratch, turning thoughts into reality with code.

- 🔭 方向 / Focus：AI 时代的基础设施（Git、浏览器、聊天）— Infrastructure for the AI era (Git, Browser, Chat)
- 🌱 信奉 / Belief：万物皆可原生实现 — Everything can be built natively
- 💬 格言 / Motto：想法要疯狂，代码要优雅 — Think crazy, code elegant

---

## 🏗️ 项目作品 / Projects

| 项目 Project | 简介 Description | 状态 Status |
|-------------|-----------------|-------------|
| [**Orcha**](https://github.com/Ink-dark/orcha) | AI 驱动的飞书编码 Agent，@ 机器人一句话自主拆解、编码、测试、审核并推到 Git 新分支 / AI-driven Feishu coding agent that plans, codes, tests, reviews and pushes to a new branch from one @-mention | 🚧 开发中 |
| [**AdapterGit (agit)**](https://github.com/Ink-dark/AdapterGit) | AI 优先、纯 Rust 原生 Git 工具 / AI-first, pure Rust Git tool | 🚧 开发中 |
| [**CrossBag**](https://github.com/bit-torch/CrossBag) | 跨平台P2P文件同步解决方案 / cross-platform sync document solution | 🚧 开发中 |
| [**MusKitty**](https://github.com/Ink-dark/MusKitty) | MusCat 浏览器 AI 智能核心 / MusCat browser AI core | 📋 规划中 |
| **HandleTokens** | AI令牌再次分发 / Token management | 💡 构思中 |
| **5Stones** | 五子棋 / Gomoku | 💡 构思中 |
| **ReChat 系列** | 聊天应用系列 / Chat app series | 💡 构思中 |
| [**crazyideas**](https://github.com/Ink-dark/crazyideas) | 疯狂点子仓库 / Crazy ideas repo | 📓 持续更新 |

---

## 🗺️ 贡献率 / Contributions

![GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Ink-dark&theme=github-compact&area=true&hide_border=true)

---

## 🛠️ 技术栈 / Tech Stack

![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

---

## 📊 统计 / Stats
<!-- ③ Streak 连续提交（换 demolab 域名，DenverCoder1 官方维护，比 heroku 稳） -->
![GitHub Streak](https://streak-stats.demolab.com?user=Ink-dark&theme=gruvbox&hide_border=true)
---

## 📫 联系我 / Contact

- GitHub: [@Ink-dark](https://github.com/Ink-dark)
- Bilibili：[墨染柒_Official 的个人主页](https://space.bilibili.com/3546815179786536)
- 邮箱 / Email: moranqi@mail.bit-torch.cn （慢回复/Slow Reply）

---

*本自述文件中所有内容使用 CC BY-NC-SA 协议授权。*
*All content in this Readme.md is licensed under CC BY-NC-SA.*
